...
def _get_series_for_node(node_id: str, max_points: int = PLOT_POINTS):
    ...
    ma_vals, std_vals, adjusted_vals, is_anom_rf, is_anom_lstm, is_anom_svm, is_anom_xgb = [], [], [], [], [], [], []
    for _, x in parsed:
        # moving average
        mv = x.get(MA_FIELD)
        ...
        try:
            ...
            adjusted = None
        ma_vals.append(mv)
        is_anom_rf.append(x.get("is_anom_rf"))
        is_anom_lstm.append(x.get("is_anom_lstm"))
        is_anom_svm.append(x.get("is_anom_svm"))
        is_anom_xgb.append(x.get("is_anom_xgb"))
...
def update_ui(_n, selected_node):
    ...
    if selected_node and selected_node in BUFFER:
        times, p_kw, v, f_hz, ma_vals, std_vals, adjusted_vals, is_anom_rf, is_anom_lstm, is_anom_svm, is_anom_xgb = _get_series_for_node(selected_node, max_points=PLOT_POINTS)
        ...
        if any(val is not None for val in adjusted_vals):
            #symbol_map={0:["circle",6,'blue'],
                        #1:["star",10,'blue'],
                        #2:["hexagon",13,'red'],
                        #3:["D",16,'red'],
                        #4:["x",20,'red']}
            #symbols = [ symbol_map[sum(preds)][0] for preds in list(zip(is_anom_rf, is_anom_lstm, is_anom_svm, is_anom_xgb))]
            #sizes = [ symbol_map[sum(preds)][1] for preds in list(zip(is_anom_rf, is_anom_lstm, is_anom_svm, is_anom_xgb))]
            #colors = [ symbol_map[sum(preds)][2] for preds in list(zip(is_anom_rf, is_anom_lstm, is_anom_svm, is_anom_xgb))]
            symbol_map = {
                0: ("circle", 6,  "blue"),
                1: ("star",   10, "blue"),
                2: ("hexagon",13,"red"),
            ...
            pred_sums = [sum(t) for t in zip(is_anom_rf, is_anom_lstm, is_anom_svm, is_anom_xgb)]

            symbols, sizes, colors = zip(*(symbol_map[s] for s in pred_sums))
